...
import (
	...
	"github.com/google/knative-gcp/hack/schema/pkg/docs"
)
...
func generateSchema(t reflect.Type) JSONSchemaProps {
	switch k := t.Kind(); k {
	case reflect.Bool:
	...
	case reflect.Struct:
		s := generateStructSchema(t, false)
		return s
...
func generateStructSchema(t reflect.Type, skipTopLevelCommon bool) JSONSchemaProps {
	s := JSONSchemaProps{
		Type:       "object",
...
func generateSliceSchema(t reflect.Type) JSONSchemaProps {
	s := JSONSchemaProps{
		Type: "array",
	}
	is := generateSchema(t.Elem())
